...
public class FenUtilities {
    ...
    public static void saveFENToFile(final File fenFile, final Board board) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fenFile))) {
            String fenString = createFENFromGame(board);
            writer.write(fenString);
            writer.newLine(); // Add a new line after the FEN string
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
